...
* **Phân lập Database:** Mỗi service có **DATABASE RIÊNG**, KHÔNG service nào được truy cập trực tiếp DB của service khác.
* **Giao tiếp qua API:** Muốn lấy/thay đổi dữ liệu của service khác $\rightarrow$ **PHẢI** gọi REST API sang service đó.
* **Ví dụ cụ thể:** `registration-service` KHÔNG có bảng `Course`, chỉ lưu `courseId` (kiểu số, không có khóa ngoại thật tới DB khác).
